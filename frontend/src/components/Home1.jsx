...
import { Upload, Button, Table, Spin, message, Tag, Select, Collapse, Progress } from "antd";
...
const Home = () => {
  ...
  const renderSkills = (skills, matchedSkills) => {
    return (
      <div>
        {skills.map((skill, index) => (
          <Tag key={index} color={matchedSkills.includes(skill) ? 'green' : 'red'}>{skill}</Tag>
        ))}
      </div>
    );
  };
      ...
      {results.length > 0 && (
        <div className="mt-8">
          ...
          <Table 
            dataSource={results} 
            columns={columns} 
            pagination={{ ...pagination, total: results.length, onChange: handlePaginationChange }}
              ...
              expandedRowRender: record => (
                <Collapse>
                  <Panel header="Details" key="1">
                    ...
                    {renderSkills(record.skills, record.matched_skills)}
                  </Panel>
                </Collapse>
              ),
